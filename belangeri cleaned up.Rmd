...
```{r}
#reminder: SSD seems to be more of a thing on mainlands than on islands; perhaps not significant for islands (also smaller sample)
...
```{r}
#temp
#CLEAN UP THESE COR.TESTS!!!

cor.test(beltemp_log$Lat, beltemp_log$temp) #strong negative correlation
cor.test(beltemp_log$Collection.Year, beltemp_log$temp) #significant but weak positive correlation
cor.test(beltemp_log$pc1, beltemp_log$temp) #significant positive correlation between temp and body size

summary(lm(pc1 ~ Collection.Year*Sex*temp, data=beltemp_log, na.action = na.omit)) #no significant interaction between temp and year, both are significant and seem to be capturing different things

#rainfall

cor.test(beltemp_log$Lat, beltemp_log$pre) #significant negative correlation as expected
plot(beltemp_log$pre ~ beltemp_log$Lat)

cor.test(beltemp_log$Collection.Year, beltemp_log$pre) #no significant correlation b/w year and rainfall

cor.test(beltemp_log$pc1, beltemp_log$pre) #significant positive correlation b/w rainfall and body size
plot(beltemp_log$pc1 ~ beltemp_log$pre)

cor.test(beltemp_log$pre, beltemp_log$temp) #significant positive correlation b/w rainfall and temp
plot(beltemp_log$pre ~ beltemp_log$temp)

#forest cover

cor.test(beltemp_log2$Lat, beltemp_log2$forests) #no sig correlation as expected

cor.test(beltemp_log2$Collection.Year, beltemp_log2$forests) #significant neg correlation b/w year and forests
plot(beltemp_log2$forests ~ beltemp_log2$Collection.Year)

cor.test(beltemp_log2$pc1, beltemp_log2$forests) #significant negative correlation b/w forest cover and body size
plot(beltemp_log2$pc1 ~ beltemp_log2$forests)

cor.test(beltemp_log2$pre, beltemp_log2$forests) #no correlation b/w rainfall and forests
...
cor.test(beltemp2$Lat, beltemp2$urb) #negative correlation between latitude and urbanization, so it's unclear whether urbanization really explains body size or whether it increases (like temperature) toward the equator and therefore increases body size
```
...
#elevation could be capturing temperature, because the temp variable is not as fine grained (55 km^2) as the elevation variable (90 x 90 m)

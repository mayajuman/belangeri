...
impmale <- imp[which(bel$Sex == "m"),]
impfemale <- imp[which(bel$Sex == "f"),]

belmale <- bel[which(bel$Sex == "m"),]
belfemale <- bel[which(bel$Sex == "f"),]
...
pcamale <- princomp(log(impmale), cor=TRUE)
pcafemale <- princomp(log(impfemale), cor=TRUE)
belmale$pc1 <- pcamale[["scores"]][,1]
belfemale$pc1 <- pcafemale[["scores"]][,1]
...
#new
belmale$Collection.Year <- as.numeric(scale(log(belmale$Collection.Year)))
belmale$Lat <- as.numeric(scale(log(belmale$Lat)))

belfemale$Collection.Year <- as.numeric(scale(log(belfemale$Collection.Year)))
belfemale$Lat <- as.numeric(scale(log(belfemale$Lat)))
...
##checking each sex separately

#male <- bel2_log[which(bel2_log$Sex == "m"),]
#female <- bel2_log[which(bel2_log$Sex == "f"),]

#belmale$Island <- relevel(belmale$Island, ref = "mainland") 
#belfemale$Island <- relevel(belfemale$Island, ref = "mainland") 

#summary(lm(pc1 ~ Collection.Year*Island*Lat, data=male, na.action = na.omit))
#summary(lm(pc1 ~ Collection.Year*Island*Lat, data=female, na.action = na.omit))

#summary(lm(pc1 ~ Collection.Year*Island*Lat, data=belmale, na.action = na.omit))
#summary(lm(pc1 ~ Collection.Year*Island*Lat, data=belfemale, na.action = na.omit))
```
...
filtered2 <- na.omit(subset(beltemp_log2, select = c(pc1, Lat, Long, Sex, elev, temp, urb, pre, prim, forests)))
filtcoords <- na.omit(subset(beltemp2, select = c(pc1, Lat, Long, Sex, elev, temp, urb, pre, prim, forests)))

coords2 = data.frame(x=filtcoords$Long, y=filtcoords$Lat)
plot(coords2)
sp2 = SpatialPoints(coords2)
data2=data.frame(resid=residuals(selected)) #rstandard for standardized residuals
spdf2 = SpatialPointsDataFrame(sp2,data2)
bubble(spdf2, "resid", maxsize = 1) #definitely clustering

library(spdep)

id2 <- row.names(filtered2)
coords2 <- data.frame(x=filtcoords$Long, y=filtcoords$Lat)
coords2 <- data.matrix(coords2)
nb2 <- knn2nb(knearneigh(coords2, k = 5), row.names = id2)
nb_w2 <- nb2listw(nb2, style = "B", zero.policy = T)

entirething3 <- (lm(pc1 ~ Sex+temp+urb+pre+prim+temp*pre, data=filtered2, na.action = na.omit))

reduced3 <- stepAIC(entirething3, direction = "backward", trace = FALSE)

lm.morantest(reduced3,nb_w2) #significant spatial autocorrelation

library(spatialreg)

#spatial error model
sem2 <- errorsarlm(formula = pc1 ~ Sex+temp+urb+pre+elev+forests+prim+temp*pre, data = filtered2, listw = nb_w2, quiet=T)
summary(sem2)

#spatial lag model
lag2 <- lagsarlm(formula = pc1 ~ Sex+temp+urb+pre+prim+elev+forests+temp*pre, data = filtered2, listw = nb_w2, quiet=T)

summary(lag2) #lower AIC, effects are conserved. phew
```
...
unscaled_pre <- (lm(pc1 ~ Sex++temp*pre, data=beltempmod, na.action = na.omit))
#these look the same
...
ggsave("FINAL FIG 4.jpg", width = 7, height = 5)
ggsave("FINAL FIG 4.pdf", width = 7, height = 5)
```


```{r}
#hierarchical partitioning analysis omitted from final MS
...
belmap <- bel
belmap$temp <- beltemp$temp[match(bel$MuseumCat., beltemp$MuseumCat.)]
...
map <- ggplot(data = world) + geom_sf() +
  theme(axis.title.x=element_blank(), axis.title.y=element_blank(), panel.grid.major = element_line(colour = "transparent"), panel.border = element_rect(colour = "black", fill=NA, size=1)) +
    geom_point(data = belmap, aes(x = Long, y = Lat, size = pc1, fill = temp), color = "black", shape = 21) + 
  coord_sf(xlim = c(86,113), ylim = c(5,30), expand = FALSE) + xlab("Longitude") + ylab("Latitude") + 
  theme(legend.position = c(0.93,0.12), axis.text = element_text(color = "black"), legend.title = element_blank(),
        legend.spacing.x = unit(1, 'mm'), legend.key.size = unit(6, 'mm'), legend.text = element_text(color="black"), legend.background = element_rect(color = "white", fill = "white", linetype = "solid", size=1)) +
...
library(magick)
ggdraw(map) + 
  draw_image("belangeri.png", x = 0.98, y = 1.07, hjust = 1, vjust = 1, width = 0.34, height = 0.34)
